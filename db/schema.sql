...
create table if not exists tasks (
  ...
  subject_id text,
  domain text not null check (domain in ('work','study')),
  title text not null,
...
create table if not exists work_records (
  ...
  type text not null check (type in ('work','study','problem','decision','knowledge','progress','communication','note')),
  title text not null,
  content text not null,
...
create table if not exists memories (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references users(id) on delete cascade,
  domain text not null check (domain in ('global','work','study')),
  type text not null check (type in ('episodic','semantic','procedural','preference','problem','decision','pattern')),
  content text not null,
  source text not null check (source in ('owner','observed','inferred')),
...
select id, user_id, domain, type, content, source, confidence, project_id, task_id, evidence_count, updated_at, last_used_at
from memories;
