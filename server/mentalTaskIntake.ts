...
function fallbackExtract(message: string): MentalTaskItem[] {
  ...
  const chunks = message
    .replace(/[，。！？；]/g, ',')
    .split(/,|、|\s*並且\s*|\s*還有\s*|\s*另外\s*|\s*以及\s*/)
    .map(s => s.trim())
    .filter(Boolean);
  ...
  for (const chunk of chunks) {
    ...
    result.push({ title: chunk.replace(/^(我明天|明天|今天|今晚|待會|我需要|我還要|幫我|我要)\s*/i, '').trim(), category, confidence: study !== work ? 'medium' : 'low' });
  }
...
export function formatMentalTaskSummary(tasks: MentalTaskItem[]) {
  ...
  const sections = (Object.keys(grouped) as MentalTaskCategory[])
    ...
    .map(category => `#### ${CATEGORY_LABELS[category]}\n${grouped[category].map((task, index) => `- **${task.title}**　（${task.confidence}）`).join('\n')}`);
