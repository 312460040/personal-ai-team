...
export interface PublicIntakeResult { category: PublicIntakeCategory; confidence: 'high'|'medium'|'low'; reason: string; projectId: string|null; method: 'ai'|'rule_fallback'; }
...
export async function classifyPublicRequest(message:string, workProjects:any[]=[]):Promise<PublicIntakeResult>{
 ...
 // Global review is a deterministic Manager-level intent. It must be checked
 // before Gemini so an AI classification can never incorrectly downgrade a
 // cross-domain read-only request to personal/work/study.
 if(isGlobalTaskReviewRequest(text)) return {category:'global',confidence:'high',reason:'這是跨工作與課業的全域任務檢視／整理需求，不應受單一 Project Context 限制。',projectId:null,method:'rule_fallback'};
