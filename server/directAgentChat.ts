...
function normalizeActions(rawActions: any[], allTasks: any[], selectedTaskIds: string[]) {
  const validIds = new Set(allTasks.map(t => String(t.id)));
  const selectedSet = new Set(selectedTaskIds);
  return Array.isArray(rawActions) ? rawActions.filter((a: any) => {
    const id = String(a?.taskId || '');
    return validIds.has(id) && (!selectedTaskIds.length || selectedSet.has(id));
  }).map((a: any) => ({
    taskId: String(a.taskId), domain: a.domain === 'study' ? 'study' : 'work', updates: sanitizeUpdates(a.updates),
...
router.post('/chat', async (req, res) => {
  ...
  try {
    ...
    const actions = normalizeActions(result.actions, allTasks, selectedTaskIds);
    return res.json({
      ...
      execution: routing.intent === 'mixed' ? {
        ...
        specialistResults,
        finalAgent: 'manager',
      } : { mode: 'single_agent', finalAgent: finalAgentId },
    });
  } catch (error) {
