...
export type AgentId = 'manager' | 'work' | 'study' | 'research' | 'personal';
export type AgentIntent = 'work' | 'study' | 'research' | 'personal' | 'mixed' | 'general';
...
export function routeManagerRequest(message: string): RouteResult {
  const text = message.trim(); const isWork = matches(WORK_PATTERNS, text); const isResearch = matches(RESEARCH_PATTERNS, text); const isStudy = matches(STUDY_PATTERNS, text); const isPersonal = matches(PERSONAL_PATTERNS, text); const requiresDataWrite = matches(WRITE_PATTERNS, text);
  const specialistCount = [isWork, isStudy, isResearch, isPersonal].filter(Boolean).length;
  if (specialistCount > 1) {
    const delegatedAgents: AgentId[] = []; if (isWork) delegatedAgents.push('work'); if (isStudy) delegatedAgents.push('study'); if (isResearch) delegatedAgents.push('research'); if (isPersonal) delegatedAgents.push('personal');
    return { primaryAgent:'manager', delegatedAgents, intent:'mixed', reason:'需求同時涉及多個生活／工作／課業領域，由 Manager 協調專業 Agent。', requiresDataWrite };
  }
...
export function buildTeamExecutionPlan(route: RouteResult): TeamExecutionPlan {
  ...
  route.delegatedAgents.forEach((agentId) => steps.push({ agentId, role:'specialist', purpose: agentId==='work'?'讀取工作 User Data，處理工作專案與任務。':agentId==='research'?'處理論文、文獻、研究方法與研究資料分析。':agentId==='study'?'讀取課業 User Data，處理學習與課業需求。':'處理日常習慣、打卡、個人備忘與生活規劃。', status:'queued' }));
