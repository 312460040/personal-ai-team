...
const PSYCHOLOGY_PATTERNS = [/心理|情緒|好累|累|疲憊|沒力|不想動|不想做|逃避|焦慮|焦躁|壓力|壓力大|煩|煩躁|崩潰|低落|難過|沮喪|緊張|害怕|擔心|動機|動力|沒動力|提不起勁|拖延|不想開始|開始不了|卡住|撐不住|內耗|自責|自我懷疑|沒自信|想放棄|不想面對|正念|mindfulness|溝通|鼓勵|倦怠|burnout/i];
...
export function routeManagerRequest(message: string): RouteResult {
  ...
  if (isPsychology) return { primaryAgent:'psychology', delegatedAgents:['psychology'], intent:'psychology', reason:'辨識為情緒、壓力、動機、拖延、自我調節或需要心理支持的自然語言表達。', requiresDataWrite };
...
export function buildTeamExecutionPlan(route: RouteResult): TeamExecutionPlan {
  ...
  route.delegatedAgents.forEach((agentId) => steps.push({ agentId, role:'specialist', purpose: agentId==='work'?'讀取工作 User Data，處理工作專案與任務。':agentId==='research'?'處理論文、文獻、研究方法與研究資料分析。':agentId==='study'?'讀取課業 User Data，處理學習與課業需求。':agentId==='personal'?'處理日常習慣、打卡、個人備忘與生活規劃。':'理解 Owner 當下情緒與狀態，運用心理學知識支持動機、壓力調節與行動啟動。', status:'queued' }));
