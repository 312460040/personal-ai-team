...
export async function loadAgentMemories(ownerId: string, agentId: string, message: string, limitPerDomain = 8) {
  ...
  const results = await Promise.all(domains.map(async (domain) => {
    const rows = await supabase(`memories?user_id=eq.${encodeURIComponent(userId)}&domain=eq.${encodeURIComponent(domain)}&select=id,domain,type,content,confidence,updated_at&order=updated_at.desc&limit=${Math.min(Math.max(limitPerDomain, 1), 20)}`);
    return (Array.isArray(rows) ? rows : []).map((row: MemoryRow) => ({ domain: safeDomain(String(row.domain || domain)), type: row.type || 'semantic', content: String(row.content || '').trim(), confidence: Number(row.confidence ?? 0.5), updatedAt: row.updated_at || null })).filter((row) => row.content);
  }));
  return results.flat();
}
...
function memoryCandidates(message: string, response: any) {
  ...
  if (persistentCue) candidates.push({ type: 'semantic', content: `Owner 長期資訊：${text.slice(0, 1000)}`, confidence: 0.92 });
  if (/我們決定|決定採用|確定採用|研究方向|研究題目|研究設計|研究方法/i.test(text)) candidates.push({ type: 'decision', content: `重要決策：${text.slice(0, 1000)}`, confidence: 0.95, domains: ['manager', 'research', 'study'] });
  if (writeAction && responseText) candidates.push({ type: 'interaction', content: `最近執行結果：${responseText.replace(/<!--AIT_TASK_BATCH:[\s\S]*?-->/g, '').trim().slice(0, 1000)}`, confidence: 0.78 });
  if (createdTasks.length) createdTasks.slice(0, 10).forEach((task: any) => { const domain: MemoryDomain = task?.subjectId || task?.subjectName ? 'study' : 'work'; candidates.push({ type: 'task_context', content: `已建立任務「${String(task.title || '未命名任務').slice(0, 200)}」${task.projectName ? `，專案：${String(task.projectName).slice(0, 120)}` : ''}${task.subjectName ? `，科目：${String(task.subjectName).slice(0, 120)}` : ''}${task.deadline ? `，截止：${String(task.deadline).slice(0, 80)}` : ''}。`, confidence: 0.9, domains: [domain] }); });
  return candidates;
}
...
export async function saveAgentMemories(ownerId: string, agentId: string, message: string, response: any) {
  ...
  const recent = await Promise.all(baseDomains.map(async (domain) => { const rows = await supabase(`memories?user_id=eq.${encodeURIComponent(userId)}&domain=eq.${encodeURIComponent(domain)}&select=content&order=updated_at.desc&limit=100`); return { domain, contents: (Array.isArray(rows) ? rows : []).map((r: any) => normalize(String(r.content || ''))) }; }));
  const savedKeys = new Set(recent.flatMap((item) => item.contents.map((content) => `${item.domain}:${content}`)));
  let saved = 0;
  for (const candidate of candidates.slice(0, 12)) for (const domain of (candidate.domains?.length ? candidate.domains : baseDomains)) {
    const content = candidate.content.trim().slice(0, 2000);
    const key = `${domain}:${normalize(content)}`;
    if (!content || savedKeys.has(key)) continue;
    await supabase('memories', { method: 'POST', body: JSON.stringify({ user_id: userId, domain, type: candidate.type, content, source: 'agent-memory', confidence: candidate.confidence, evidence_count: 1 }) });
    savedKeys.add(key); saved += 1;
  }
...
export function memoryHeader(memories: Array<{ domain: string; type: string; content: string }>) {
  ...
  const grouped = memories.reduce<Record<string, string[]>>((acc, memory) => { const key = memory.domain || 'manager'; (acc[key] ||= []).push(`- [${memory.type}] ${memory.content}`); return acc; }, {});
  ...
  express.application.post = function agentMemoryPost(path: any, ...handlers: any[]) {
    ...
    const wrapped = handlers.map((handler: any) => {
      ...
      return async function agentMemoryHandler(this: any, req: any, res: any, next: any) {
        ...
        } catch (error) { console.warn('[Agent Memory] load skipped:', error instanceof Error ? error.message : String(error)); }
        const originalJson = res.json.bind(res);
        res.json = (payload: any) => { Promise.resolve(saveAgentMemories(ownerId, agentId, message, payload)).catch((error) => console.warn('[Agent Memory] save skipped:', error instanceof Error ? error.message : String(error))); return originalJson(payload); };
