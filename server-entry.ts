...
function buildMentalTaskPayloads(mentalTasks: any[], message: string, context: any) {
  ...
  if (dateMatch) {
    ...
    deadlineBase = new Date(y, m - 1, d, 18, 0, 0);
  }
  ...
  const createdWorkTasks: any[] = [];
  const createdStudyTasks: any[] = [];
  const blocked: any[] = [];
  ...
  mentalTasks.forEach((task: any, index: number) => {
    ...
    const common = {
      ...
      deadline,
      assignee: '本人',
      notes: `由 Owner 透過 Manager 對話建立。原始指令：「${message.trim()}」`,
    ...
    if (task.category === 'work') {
      if (!matchedProject) {
        blocked.push({ title, category: 'work', reason: '目前無法安全唯一匹配工作專案；Manager 不猜測 Project。' });
        return;
      }
      ...
      if (userSubjects.length !== 1) {
        blocked.push({ title, category: 'study', reason: userSubjects.length === 0 ? '尚未建立 User Study Subject；需要先指定科目。' : '目前有多個 User Study Subject；需要指定科目後才能安全建立。' });
        return;
      }
  ...
  if (path === '/api/agent/chat' && handlers.length) {
    const wrappedHandlers = handlers.map((handler: any) => {
      ...
      return async function publicIntakeHandler(this: any, req: any, res: any, next: any) {
        ...
        req.body.context = {
          ...context,
          publicIntake: { ...result, routingInstruction, mentalTasks },
          currentContext: {
        ...
        if (result.category === 'global') {
          ...
          return res.json({
            intentType: result.mode === 'daily_review' ? 'DAILY_REVIEW' : result.mode === 'tomorrow_plan' ? 'TOMORROW_PLAN' : 'GLOBAL_TASK_REVIEW',
            delegatedAgents: ['work', 'study'],
            activityLogs: [{ id: `act-manager-${Date.now()}`, timestamp: new Date().toISOString(), stepIndex: 1, fromAgent: 'manager', action: modeLabel, summary: '跨工作與課業整理 User Tasks', detail: 'Manager-level 唯讀分析；等待 Owner 審核後才進入安排。', status: 'completed', durationMs: 0 }],
            workOutput: report,
            studyOutput: '',
            finalSynthesisMarkdown: report,
            proposedTimeBlocks: [],
            createdTaskPayload: null,
            durationTotalMs: 0,
            publicIntake: req.body.context.publicIntake
          });
        }

        if (arrangingTasks) {
          const payloads = buildMentalTaskPayloads(mentalTasks, message, context);
          const totalCreated = payloads.createdWorkTasks.length + payloads.createdStudyTasks.length;
          const grouped = [
            payloads.createdWorkTasks.length ? `#### 💼 工作\n${payloads.createdWorkTasks.map((t: any) => `- **${t.title}**　｜${t.projectName}｜${t.priority}｜截止 ${t.deadline}`).join('\n')}` : '',
          ...
          return res.json({
            intentType: 'MENTAL_TASK_INTAKE',
            delegatedAgents: [...new Set(mentalTasks.map((t: any) => t.category).filter((c: any) => c === 'work' || c === 'study'))],
            activityLogs: [{ id: `act-task-intake-${Date.now()}`, timestamp: new Date().toISOString(), stepIndex: 1, fromAgent: 'manager', action: '拆分新任務並分類後建立', summary: `Manager 建立 ${totalCreated} 筆 User Task`, detail: '依自然語言拆分為工作／課業，建立前檢查 Project／Subject 歸屬。', status: 'completed', durationMs: 0 }],
            workOutput: payloads.createdWorkTasks.map((t: any) => t.title).join('\n'),
            studyOutput: payloads.createdStudyTasks.map((t: any) => t.title).join('\n'),
            finalSynthesisMarkdown: report,
            proposedTimeBlocks: [],
            createdWorkTasks: payloads.createdWorkTasks,
            createdStudyTasks: payloads.createdStudyTasks,
            createdTaskPayload: null,
            durationTotalMs: 0,
            publicIntake: req.body.context.publicIntake
          });
        ...
        if (mentalTasks.length) {
          const categories = [...new Set(mentalTasks.map((task: any) => task.category))];
          const delegated = categories.filter((category: any) => category === 'work' || category === 'study');
          const action = '心中任務收件匣：先拆解並分類，尚未建立正式 Task';
          return res.json({
            intentType: 'MENTAL_TASK_INTAKE',
            delegatedAgents: delegated,
            activityLogs: [{ id: `act-mental-${Date.now()}`, timestamp: new Date().toISOString(), stepIndex: 1, fromAgent: 'manager', action, summary: `辨識 ${mentalTasks.length} 個 Owner 明確提到的任務`, detail: '只使用本次訊息；不讀取其他任務來補齊、不自動建立或修改正式 Task。', status: 'completed', durationMs: 0 }],
            workOutput: mentalTasks.filter((task: any) => task.category === 'work').map((task: any) => task.title).join('\n'),
            studyOutput: mentalTasks.filter((task: any) => task.category === 'study').map((task: any) => task.title).join('\n'),
            finalSynthesisMarkdown: `${mentalTaskSummary}\n\n### 📌 下一步\n你可以直接告訴 Manager：**「把這些建立成正式任務」**或**「幫我安排明天時間」**。在你明確要求前，這些只會停留在「心中任務」，不會寫入 Task 資料。`,
            proposedTimeBlocks: [],
            createdTaskPayload: null,
            durationTotalMs: 0,
            publicIntake: req.body.context.publicIntake
