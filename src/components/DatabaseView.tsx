...
const TABLE_LABELS: Record<string, string> = { users: '使用者', projects: 'Projects', tasks: 'Tasks', conversations: '對話紀錄', work_records: '工作紀錄', memories: 'Memory', focus_sessions: 'Focus Session', calendar_events: 'Calendar', diagnosis_records: 'Diagnosis', adaptive_proposals: 'Adaptive Proposal' };
...
export default function DatabaseView() {
  ...
  const loadTables = async () => {
    setError(null);
    try { const response = await fetch('/api/persistence/tables', { headers }); if (!response.ok) throw new Error(`資料庫連線失敗（HTTP ${response.status}）`); const data = await response.json(); setTables(Array.isArray(data) ? data : []); }
    catch (e) { setError(e instanceof Error ? e.message : '無法連線到資料庫 API'); }
  };
  const loadRows = async (table: string) => {
    setLoading(true); setError(null);
    try { const response = await fetch(`/api/persistence/tables/${encodeURIComponent(table)}?limit=100`, { headers }); if (!response.ok) throw new Error(`無法讀取 ${table}（HTTP ${response.status}）`); const data = await response.json() as TableResponse; setRows(Array.isArray(data.rows) ? data.rows : []); }
  ...
  return <div className="mx-auto max-w-7xl px-3 sm:px-5 pt-6">
    ...
    {error && <div className="mt-5 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800"><AlertCircle className="mt-0.5 w-4 h-4 shrink-0" /><div><b>目前無法讀取資料庫</b><p className="mt-1">{error}。如果你是在 GitHub Pages 預覽，後端 persistence API 尚未提供時會看到這個狀態；本機啟動 Server 並設定 Supabase 環境變數後即可查看。</p></div></div>}
