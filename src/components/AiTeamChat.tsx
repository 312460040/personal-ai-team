...
import { ChatMessage, AgentInfo, StructuredTimeBlock, WorkTask, StudyTask, WorkProject, StudySubject, AgentActivityLog, AgentId } from '../types';
...
export const AiTeamChat: React.FC<AiTeamChatProps> = ({ messages, onSendMessage, isLoading, onApplyScheduleToToday, currentActiveAgents, agentRegistry = [], workTasks = [], studyTasks = [], workProjects = [], studySubjects = [], onToggleWorkTask = () => {}, onToggleStudyTask = () => {}, onUpdateWorkTask = () => {}, onUpdateStudyTask = () => {} }) => {
  ...
  const buildActivityLogs = (data: any, prompt: string): AgentActivityLog[] => {
    const routing = data?.routing;
    if (!routing) return [];
    const effective = String(routing.effectiveAgentId || data.agentId || 'manager') as AgentId;
    const delegated = Array.isArray(routing.delegatedAgents) ? routing.delegatedAgents.filter((id: any) => ['manager', 'work', 'study'].includes(String(id))) as AgentId[] : [];
    const logs: AgentActivityLog[] = [{ id: `act-route-${Date.now()}`, timestamp: new Date().toISOString(), stepIndex: 1, fromAgent: 'manager', toAgent: effective, action: 'Manager 分析需求並分流', summary: routing.reason || 'Manager 已完成需求判斷。', detail: `需求：「${prompt}」｜${routing.requiresDataWrite ? '可能需要資料寫入' : '先進行唯讀分析或對話'}`, status: 'completed', durationMs: 0 }];
    if (effective !== 'manager') {
      logs.push({ id: `act-agent-${Date.now()}`, timestamp: new Date().toISOString(), stepIndex: 2, fromAgent: effective, action: '專業 Agent 執行', summary: effective === 'work' ? 'Work Agent 讀取工作 User Data 並處理需求。' : 'Study Agent 讀取課業／研究 User Data 並處理需求。', detail: Array.isArray(data.actions) && data.actions.length ? `已提出 ${data.actions.length} 項資料更新。` : '本次未產生資料更新。', status: 'completed', durationMs: 0 });
      logs.push({ id: `act-return-${Date.now()}`, timestamp: new Date().toISOString(), stepIndex: 3, fromAgent: effective, toAgent: 'manager', action: '回傳結果給 Manager', summary: '專業 Agent 已完成本輪處理，結果交回 Manager。', status: 'completed', durationMs: 0 });
    }
    if (Array.isArray(data.actions) && data.actions.length) {
      logs.push({ id: `act-write-${Date.now()}`, timestamp: new Date().toISOString(), stepIndex: logs.length + 1, fromAgent: effective, action: '套用 Task 更新', summary: `已對 ${data.actions.length} 筆選定 Task 執行實際更新。`, detail: data.actions.map((a: any) => String(a.taskId)).join('、'), status: 'completed', durationMs: 0 });
    }
    return logs;
  };
