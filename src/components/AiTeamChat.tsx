...
export const AiTeamChat: React.FC<AiTeamChatProps> = ({ messages, onSendMessage, isLoading, onApplyScheduleToToday, currentActiveAgents, agentRegistry = [], workTasks = [], studyTasks = [], onToggleWorkTask = () => {}, onToggleStudyTask = () => {}, onUpdateWorkTask = () => {}, onUpdateStudyTask = () => {} }) => {
  ...
  const displayMessages = directMessages.length ? directMessages : (selectedAgent.id === 'manager' ? messages : []);
  ...
  const applyActions = (actions: any[]) => {
    actions.forEach(action => {
      ...
      const sourceTask = domain === 'study' ? studyTasks.find(t => t.id === action.taskId) : workTasks.find(t => t.id === action.taskId);
      if (!sourceTask) return;
      const updates = action.updates || {};
      const next: any = { ...sourceTask, ...updates, source: sourceTask.source, createdBy: sourceTask.createdBy };
  ...
  const handleDirectSend = async (text: string) => {
    ...
    setDirectMessages(prev => [...prev, userMessage]); setDirectLoading(true);
    try {
      const response = await fetch(apiUrl('/api/agent/direct/chat'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: prompt, agentId: selectedAgent.id, agentName: selectedAgent.name, agentRole: selectedAgent.role, history, context: { workTasks, studyTasks, selectedTaskIds, currentContext: { workspaceId: selectedAgent.id === 'study' ? 'study' : 'work' } } }) });
      if (!response.ok) throw new Error(`Direct agent chat returned ${response.status}`);
      const data = await response.json();
      if (Array.isArray(data.actions) && data.actions.length) {
        applyActions(data.actions);
        setSelectedTaskIds(prev => prev.filter(id => !data.actions.some((a: any) => a.taskId === id)));
        confetti({ particleCount: 30, spread: 55, origin: { y: 0.85 } });
      }
      ...
      setDirectMessages(prev => [...prev, { id: `direct-error-${Date.now()}`, sender: 'agent', agentId: selectedAgent.id, agentName: selectedAgent.name, agentRole: selectedAgent.role, text: '目前無法連線到 AI 員工。請確認 Render Backend 與 Gemini API 是否正常。', timestamp: new Date().toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit', hour12: false }) }]);
