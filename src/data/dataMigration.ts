...
const VERSION = 'ait_data_schema_v12';
...
function addMissing<T extends { id: string }>(items: T[], incoming: T[]): boolean { const ids = new Set(items.map(x => x.id)); let changed = false; for (const x of incoming) if (!ids.has(x.id)) { items.push(x); ids.add(x.id); changed = true; } return changed; }
...
export function migrateWorkData(): { changed:boolean; message?:string } {
  ...
  let projects = read<WorkProject>(PROJECTS_KEY).filter(p=>!demoProjectTitles.has(p.title));
  ...
  // Collapse legacy duplicate client records by canonical IDs.
  for (const [title,id,type] of [['李總醫療體系',PARENT_ID,'client_root'],['綜合醫院','proj-client-general-hospital','client_root'],['旅遊業的客戶','proj-client-travel','client_root']] as const) {
    ...
    const canonical = matches.find(p=>p.id===id) || matches[0];
    if (canonical) {
  ...
  for (const site of LI_SITES) {
    const id=SITE_IDS[site]; const matches=projects.filter(p=>p.title===site || p.id===id);
    const canonical=matches.find(p=>p.id===id) || matches[0] || baselineProject(id,site,'李總醫療體系｜據點','client_site',PARENT_ID);
    for (const duplicate of matches) if (duplicate.id!==canonical.id) { aliases.set(duplicate.id,id); projects=projects.filter(p=>p.id!==duplicate.id); }
  ...
  // Remap tasks from duplicate projects and classify every Li Medical task by title/notes.
  const projectIds = new Set(projects.map(p=>p.id));
  const deduped = new Map<string,WorkTask>();
  for (const task of tasks) {
    ...
    const key=`${t.projectId}|${String(t.title||'').trim().toLowerCase()}`;
    const existing=deduped.get(key); if(!existing || String(t.notes||'').length>String(existing.notes||'').length) deduped.set(key,t);
  }
  ...
  const before=tasks.length; addMissing(tasks,required);
  const changed=projects.length!==read<WorkProject>(PROJECTS_KEY).length||tasks.length!==before||aliases.size>0||previous!=='1';
  write(PROJECTS_KEY,projects); write(TASKS_KEY,tasks); localStorage.setItem(VERSION,'1');
  return {changed,message:'工作管理資料已完成去重、階層化與任務分類'};
}
