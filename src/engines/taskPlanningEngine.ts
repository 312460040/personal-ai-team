...
export interface DecompositionInput {
  ...
  taskType: PlanTaskType;
  project?: WorkProject;
  sourceTask?: WorkTask | StudyTask;
  requestedSteps?: string[];
  now?: Date;
}

/** 無 LLM 也能運作的 deterministic 任務拆解器；未來可作為 LLM 結果的驗證層。 */
export function decomposeTask(input: DecompositionInput): ExecutionPlan {
  ...
  const planSteps = steps.map((title, index) => ({
    ...
    estimatedHours: index === steps.length - 1
      ? Math.max(0.25, Number((baseHours - perStep * (steps.length - 1)).toFixed(2)))
      : perStep,
  ...
  return {
    ...
    projectId: input.taskType === 'work'
      ...
      : undefined,
    subjectId: input.taskType === 'study'
      ? (input.sourceTask && 'subjectId' in input.sourceTask ? input.sourceTask.subjectId : undefined)
      : undefined,
    status: 'draft',
    steps: planSteps,
    totalEstimatedHours: Number(planSteps.reduce((sum, step) => sum + step.estimatedHours, 0).toFixed(2)),
...
function inferSteps(title: string, taskType: PlanTaskType): string[] {
  ...
  if (taskType === 'work') {
    if (/(影片|短影音|video|reel)/i.test(lower)) return ['確認需求與目標', '整理素材與參考', '建立腳本／製作草稿', '內部檢查與修改', '提交客戶確認／發布'];
...
export function validateExecutionPlan(plan: ExecutionPlan): string[] {
  ...
  if (plan.steps.length === 0) errors.push('計畫至少需要一個步驟。');
  if (plan.steps.some(step => step.estimatedHours <= 0)) errors.push('所有步驟的預估時間必須大於 0。');
  const ids = new Set(plan.steps.map(step => step.id));
  plan.steps.forEach(step => step.dependsOn.forEach(dep => {
    if (!ids.has(dep)) errors.push(`步驟「${step.title}」依賴不存在的步驟。`);
  }));
  return errors;
}
